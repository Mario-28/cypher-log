  ...
  static openEditor(entry=null, template=null) {
    ...
    const saveDocument = async ({autosave = false, force = false} = {}) => {
      ...
      try {
        ...
        CypherLog.renderShelf(); if (autosave) ui.notifications.info("CYPHER LOG autosaved.");
        return savedEntry;
      } catch (error) {
...
Hooks.once("ready", () => {
  ...
  }); Hooks.on("createJournalEntry", entry => { if (isCypherLog(entry)) setTimeout(() => CypherLog.renderShelf(), 50); });
Hooks.on("updateJournalEntry", entry => { if (isCypherLog(entry)) CypherLog.renderShelf(); });
